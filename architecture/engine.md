...
`EffectEngine` provides a global `Uint8Array(512)` named `dmxBuffer` array. 

### Why a `Uint8Array`?
1. **Performance & Memory**: In the first version of Pixelmapper, Vue's deep reactivity system monitored every individual channel's value. When running effects on 40+ fixtures (resulting in hundreds of channel changes per frame at 60fps), Vue's reactivity overhead crippled framerates. Moving to a native typed array bypasses Vue's proxy watchers entirely, offering blazing-fast C-level memory access.
...
2. **Compute Base/Chaser Values** — computes interpolated sub-frame channel bases via the `chaserConfig`.
3. **Apply effects** — for each fixture, each effect renders a normalized wave shape `[-1, 1]`. The engine maps this shape to a safe DMX range, then offsets the channel value.
4. **Buffer Write Phase** — final rounded values are instantly copied to their index inside the memory-efficient `dmxBuffer`.
